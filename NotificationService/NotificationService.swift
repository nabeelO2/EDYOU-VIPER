...
//  NotificationService
//
//  Created by imac3 on 14/11/2023.
//
...
class NotificationService: UNNotificationServiceExtension {
    ...
    func debug(_ data: Any...) {
        os_log("%{public}@", log: OSLog(subsystem: Bundle.main.bundleIdentifier!, category: "EdyouPush"), "\(Date()): \(data)");
    }
...
class ExtensionNotificationManagerProvider: NotificationManagerProvider {
    ...
    func avatar(on account: BareJID, for sender: BareJID) -> INImage? {
        ...
        let url = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.edyou.shared")!.appendingPathComponent("Library", isDirectory: true).appendingPathComponent("Caches", isDirectory: true).appendingPathComponent("avatars", isDirectory: true).appendingPathComponent(hash);
