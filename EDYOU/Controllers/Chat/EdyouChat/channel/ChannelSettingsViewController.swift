        ...
        if indexPath.section == 1 && indexPath.row == 0 {
            ...
            controller.sink(receiveValue: { [weak self] value in
                self?.channel.updateOptions({ options in
                    ...
                    if let account = self?.channel.account, let pushModule = self?.channel.context?.module(.push) as? XMPPPushNotificationsModule, let pushSettings = pushModule.pushSettings {
