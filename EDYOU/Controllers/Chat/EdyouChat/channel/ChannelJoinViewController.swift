...
class ChannelJoinViewController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.nicknameField.text = Cache.shared.user?.name?.completeName ?? ""
        self.nicknameField.isUserInteractionEnabled = false
        self.nameField.isUserInteractionEnabled = false
        self.bookmarkCreateSwitch.isUserInteractionEnabled = false
        self.bookmarkAutojoinSwitch.isUserInteractionEnabled = false
        bookmarkCreateSwitch.isOn = true;
        bookmarkAutojoinSwitch.isOn = true;
        switch action {
        case .join:
    ...
    @IBAction func joinClicked(_ sender: Any) {
        ...
        case .create(let isPublic, let invitationOnly, let description, let avatar):
            let avatar1 = avatar == nil ? UIImage(named: "pushNotifications") : avatar
            self.create(name: name!, description: description, nick: nick, isPublic: isPublic, invitationOnly: invitationOnly, avatar: avatar1);
        }
     }
...
//            form.addField(TextSingleField(name: "muc#roomconfig_roomdesc", value: "Room description"))
//            form.addField(BooleanField(name: "muc#roomconfig_allowinvites", value: false))
//            form.addField(TextSingleField(name: "muc#roomconfig_maxusers", value: "2"))
//            form.addField(BooleanField(name: "muc#roomconfig_anonymous", value: false))
//            form.addField(BooleanField(name: "muc#roomconfig_anonymous", value: false))
//            form.addField("muc#roomconfig_whois").addValue("participants")
//            form.addField(TextSingleField(name: "muc#roomconfig_roomdesc", value: channelDescription));
            form.addField(TextSingleField(name: "muc#roomconfig_whois", value:  "moderators"))
                        ...
                        case .created(let room), .joined(let room):
                            ...
                            let finished = {
                                DispatchQueue.main.async {
                                    self?.operationEnded();
                                    self?.dismiss(animated: true, completion: nil);
                                    self?.onConversationJoined?(room as! XMPPRoom);
                                    self?.renameChatGroup(mucModule,JID(room.jid),name)
                                }
                            }
                            switch configResult {
                            case .success(_):
    ...
    private func renameChatGroup(_ mucModule : MucModule,_ roomJid : JID  ,_ name : String){
        mucModule.getRoomConfiguration(roomJid: roomJid, completionHandler: { result in
            switch result {
            case .success(let form):
                (form.getField(named: "muc#roomconfig_roomname") as? TextSingleField)?.value = name;
                mucModule.setRoomConfiguration(roomJid: roomJid, configuration: form, completionHandler: { result in
                    DispatchQueue.main.async {
//                        self.hideIndicator();
                        switch result {
                        case .success(_):
            ...
            client.module(.muc).join(roomName: room.localPart!, mucServer: room.domain, nickname: nick, password: password).handle({ result in
                ...
                case .success(let joinResult):
                    ...
                    case .created(let room), .joined(let room):
                        ...
                        (room as! XMPPRoom).registerForTigasePushNotification(true, completionHandler: { (result) in
