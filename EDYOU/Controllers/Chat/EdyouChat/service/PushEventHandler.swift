...
open class PushEventHandler: XmppServiceExtension {
    ...
    func updatePushRegistration(for client: XMPPClient, features: [String], shouldEnable: Bool) {
        ...
        let pushModule = client.module(.push) as! XMPPPushNotificationsModule;
        let hasPush = features.contains(XMPPPushNotificationsModule.PUSH_NOTIFICATIONS_XMLNS);
    ...
    func updateAccountPushSettings(for account: BareJID) {
        ...
        if let client = XmppService.instance.getClient(for: account), client.state == .connected(), let pushModule = client.module(.push) as? XMPPPushNotificationsModule, let pushSettings = pushModule.pushSettings {
            pushModule.reenable(pushSettings: pushSettings, completionHandler: { result in
