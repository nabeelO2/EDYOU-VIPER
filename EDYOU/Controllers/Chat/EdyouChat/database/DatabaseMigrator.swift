...
public class DatabaseMigrator: DatabaseSchemaMigrator {
    ...
    public func upgrade(database: DatabaseWriter, newVersion version: Int) throws {
        ...
        case 14:
            ...
            let roomsToUpdate: [(Int,XMPPRoomOptions)] = try database.select("SELECT c.id, c.name, c.nickname, c.password FROM chats c WHERE c.type = 1 AND c.nickname IS NOT NULL", cached: false).mapAll({ c -> (Int,XMPPRoomOptions)? in
                ...
                var options: XMPPRoomOptions = c.object(for: "options") ?? XMPPRoomOptions();
                if options.nickname.isEmpty {
                    let notifications = options.notifications;
                    options = XMPPRoomOptions(nickname: nickname, password: password);
