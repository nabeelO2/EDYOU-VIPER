...
class SettingsStore {
    ...
    @UserDefaultsSetting(key: "RosterDisplayHiddenGroup", defaultValue: true)
    ...
    @UserDefaultsSetting(key: "SharingViaHttpUpload", defaultValue: true)
    ...
    @UserDefaultsSetting(key: "XmppPipelining", defaultValue: true)
    var xmppPipelining: Bool;
    
    @UserDefaultsSetting(key: "enableBookmarksSync", defaultValue: true)
    var enableBookmarksSync: Bool;
    @UserDefaultsRawSetting(key: "messageEncryption", defaultValue: ConversationEncryption.omemo)
    ...
    @UserDefaultsSetting(key: "enablePush", defaultValue: true)
    var enablePush: Bool?;
    
    public static let sharedDefaults = UserDefaults(suiteName: "group.edyou.shared")!;
                ...
                if k.starts(with: "upload-") {
                    ...
                    if let timestamp = (v as? [String: Any])?["timestamp"] as? Date {
                        if timestamp < removeOlder {
                            ...
                            let localUploadDirUrl = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.edyou.shared")!.appendingPathComponent("upload", isDirectory: true).appendingPathComponent(hash, isDirectory: false);
