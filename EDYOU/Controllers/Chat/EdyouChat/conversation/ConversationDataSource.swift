...
class ConversationDataSource {
    ...
    var count: Int {
     ...
     var oldestEntry: ConversationEntry?;
     var entries: [ConversationEntry] = [];
     var entriesCount: Int = 0;
     var markers: [ConversationEntry] = [];
     var unreads: [ConversationEntry] = [];
    ...
    private func updateStore(scrollTo: ScrollTo = .none, completionHandler: (()->Void)? = nil) {
        ...
        let newStore1 = (entries + unreads).sorted();
        let first = newStore.firstIndex { obj in
            obj.id == 9223372036854775807
        } ?? -1
        if markers.count > 0{
            DispatchQueue.main.asyncAfter(deadline: .now()+0.5) {
                
                switch self.markers[0].payload {
                
                case .marker(type: let type, senders: _):
                    print(type)
                    if type == .displayed {
                        print("displayed : \(first)")
                        self.delegate?.markAsReadUpToIndex(first)
                    }
                    else{
