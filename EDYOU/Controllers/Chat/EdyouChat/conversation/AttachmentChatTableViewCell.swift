...
//        self.customView?.backgroundColor = self.backgroundColor;
        ...
        if !(appendix.mimetype?.starts(with: "audio/") ?? false), let localUrl = DownloadStore.instance.url(for: "\(item.id)") {
            ...
            NSLayoutConstraint.activate([
                ...
                linkView.bottomAnchor.constraint(equalTo: self.customView.bottomAnchor, constant: -30),
            ...
            NSLayoutConstraint.activate([
                ...
                customView.bottomAnchor.constraint(equalTo: attachmentInfo.bottomAnchor,constant:30)
            ])
            ...
            case .new:
                if DownloadStore.instance.url(for: "\(item.id)") == nil {
                    ...
                    if sizeLimit > 0 {
                        if (DBRosterStore.instance.item(for: item.conversation.account, jid: JID(item.conversation.jid))?.subscription ?? .none).isFrom || (DBChatStore.instance.conversation(for: item.conversation.account, with: item.conversation.jid) as? XMPPRoom != nil) {
...
//            details.adjustsFontForContentSizeCategory = true;
            ...
            actionButton.tintColor = UIColor.systemTintColor;
