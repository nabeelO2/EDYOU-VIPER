        ...
        if let account = self.account {
            DBVCardStore.instance.vcard(for: account, completionHandler: { vcard in
                ...
                DispatchQueue.main.async {
                    if let url = XmppUri(jid: JID(account), action: nil, dict: dict).toURL()?.absoluteString, let qrCode = QRCode(string: url, scale: 10, foregroundColor: UIColor(named: "qrCodeForeground")!, backgroundColor: UIColor(named: "qrCodeBackground")!) {
