...
class ContactViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationController?.navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .refresh,target: self, action: #selector(refreshVCard(_:)))
    ...
    @objc func refreshVCard(_ sender: UIBarButtonItem) {
        refreshVCard();
    }
...
//            return cell;
    ...
    @objc func muteContactChanged(_ sender: UISwitch) {
        ...
        chat?.updateOptions({ (options) in
            ...
            if let pushModule = XmppService.instance.getClient(for: account)?.module(.push) as? XMPPPushNotificationsModule, let pushSettings = pushModule.pushSettings {
