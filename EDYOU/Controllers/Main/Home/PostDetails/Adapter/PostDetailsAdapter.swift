...
class PostDetailsAdapter: NSObject {
    
    // MARK: - Properties
    weak var tableView: UITableView!
    var isLoading = true

    var parent: PostDetailsController? {
        return tableView.viewContainingController() as? PostDetailsController
    }
    var post: Post {
        didSet {
...
extension PostDetailsAdapter: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            if post.medias.count > 0 {
                let cell = tableView.dequeueReusableCell(withIdentifier: PostDetailsImageCell.identifier, for: indexPath) as! PostDetailsImageCell
                ...
                let dimenstionW = Double(dimenstion?.0 ?? 0)
                let dimenstionH = Double(dimenstion?.1 ?? 0)
                let ratio = dimenstionW > 0 ? Double(dimenstionH / dimenstionW) : Double(1.1)
                ...
                let cell = tableView.dequeueReusableCell(withIdentifier: PostDetailsTextWithBgCell.identifier, for: indexPath) as! PostDetailsTextWithBgCell
                cell.actionDelegate = self
                cell.indexPath = indexPath
                cell.setData(post)
                return cell
            }
            let cell = tableView.dequeueReusableCell(withIdentifier: PostDetailsTextCell.identifier, for: indexPath) as! PostDetailsTextCell
            cell.setData(post)
            cell.actionDelegate = self
            cell.indexPath = indexPath
            return cell
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section == 1 && self.postComments.count > 0 {
    ...
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        if section == 1 && self.postComments.count > 0 { return 44}
        else  { return 0 }
    }
...
extension PostDetailsAdapter: PostCellActions {
...
extension PostDetailsAdapter: CommentActionProtocol {
