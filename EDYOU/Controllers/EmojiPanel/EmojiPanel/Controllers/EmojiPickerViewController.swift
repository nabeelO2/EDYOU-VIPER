...
open class EmojiPickerViewController: UIViewController {
    ...
    open var onPickEmoji: ((Emoji) -> Void)?
    
    open var onDismissController: (() -> Void)?
    ...
    convenience init(onPick: @escaping (Emoji)->Void) {
        ...
        configuration.headerAppearance.textAlignment = .left

        // Changing each cell appeanrance.
        let cellSize = UIScreen.main.bounds.width / 6 - 10
        configuration.cellAppearance.size = .init(width: cellSize, height: cellSize)


        // Receiveing events from the picker view controller.
//        emojiPickerViewController.delegate = delegate
        self.init(configuration: configuration)
        self.onPickEmoji = onPick
    }
    ...
    open override func viewDidDisappear(_ animated: Bool) {
        ...
        if let dismiss = onDismissController{
            dismiss()
        }
    ...
    private func setupView() {
        ...
        searchBar.searchTextField.placeholder = String(localized: "search_emoji", bundle: .main, comment: "SearchBar placeholder text: hints what the user should enter in.")
        searchBar.returnKeyType = .search
        searchBar.searchTextField.clearButtonMode = .whileEditing
        searchBar.searchBarStyle = .minimal
        searchBar.delegate = self
        searchBar.accessibilityLabel = String(localized: "ax_searchbar_label", bundle: .main, comment: "Accessibility label: speaks what the purpose of the search field.")
    ...
    private func setupDataSource() {
        ...
        let cellRegistration = UICollectionView.CellRegistration<UICollectionViewCell, EmojiPickerItem> { [unowned self] cell, indexPath, item in
            ...
            if let emoji = item.emoji {
                ...
                contentConfiguration.font = UIFont.systemFont(ofSize: 30)
                ...
                var contentConfiguration = LabelContentConfiguration()
                contentConfiguration.text = String(localized: "no_results", bundle: .main, comment: "Empty state text: feedbacks no-results to the user.")
...
extension EmojiPickerViewController: UIScrollViewAccessibilityDelegate {
    ...
    public func accessibilityScrollStatus(for scrollView: UIScrollView) -> String? {
        ...
        return String(format: String(localized: "ax_collection_view_scroll_status", bundle: .main, comment:  "Accessibility scroll status: speaks the current page index and the visible section labels."), currentPage as NSNumber, totalPages as NSNumber, visibleSectionsLabel as NSString)
    }
