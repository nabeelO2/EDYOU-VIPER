...
class EDCropVC: UIViewController {
    
    public var didFinishCropping: ((UIImage) -> Void)?
    
    override var prefersStatusBarHidden: Bool { return EDConfig.hidesStatusBar }
    
    private let originalImage: UIImage
    private let pinchGR = UIPinchGestureRecognizer()
    private let panGR = UIPanGestureRecognizer()
    
    private let v: EDCropView
    override func loadView() { view = v }
    
    required init(image: UIImage) {
        v = EDCropView(image: image)
        originalImage = image
        super.init(nibName: nil, bundle: nil)
        self.title = EDConfig.wordings.crop
    }
    ...
    func setupToolbar() {
        let cancelButton = UIBarButtonItem(title: EDConfig.wordings.cancel,
                                           style: .plain,
                                           target: self,
                                           action: #selector(cancel))
        cancelButton.tintColor = .ypLabel
        cancelButton.setFont(font: EDConfig.fonts.leftBarButtonFont, forState: .normal)
        
        let flexibleSpace = UIBarButtonItem(barButtonSystemItem: .flexibleSpace, target: nil, action: nil)
        
        let saveButton = UIBarButtonItem(title: EDConfig.wordings.save,
                                           style: .plain,
                                           target: self,
                                           action: #selector(done))
        saveButton.setFont(font: EDConfig.fonts.rightBarButtonFont, forState: .normal)
...
extension EDCropVC: UIGestureRecognizerDelegate {
