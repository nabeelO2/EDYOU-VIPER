...
extension AVAsset {
    func assetByTrimming(startTime: CMTime, endTime: CMTime) throws -> AVAsset {
        ...
        do {
            ...
            throw EDTrimError("Error during composition", underlyingError: error)
        }
    ...
    func export(to destination: URL,
        ...
        guard let exportSession = AVAssetExportSession(asset: self, presetName: EDConfig.video.compression) else {
            EDLog("AVAsset -> Could not create an export session.")
            return nil
        }
        
        exportSession.outputURL = destination
        exportSession.outputFileType = EDConfig.video.fileType
