...
class EDVideoProcessor {
    ...
    static func makeVideoPathURL(temporaryFolder: Bool, fileName: String) -> URL {
        ...
        if temporaryFolder {
            let outputPath = "\(NSTemporaryDirectory())\(fileName).\(EDConfig.video.fileType.fileExtension)"
            ...
            guard let documentsURL = FileManager
                ...
                EDLog("Can't get the documents directory URL")
                return URL(fileURLWithPath: "Error")
            }
            outputURL = documentsURL.appendingPathComponent("\(fileName).\(EDConfig.video.fileType.fileExtension)")
        }
        ...
        if fileManager.fileExists(atPath: outputURL.path) {
            do {
                ...
                EDLog("Can't remove the file for some reason.")
            }
        }
    ...
    static func cropToSquare(filePath: URL, completion: @escaping (_ outputURL: URL?) -> Void) {
        ...
        _ = asset.export(to: outputPath, videoComposition: videoComposition, removeOldFile: true) { exportSession in
            DispatchQueue.main.async {
                ...
                case .failed:
                    EDLog("Export of the video failed: \(String(describing: exportSession.error))")
                    completion(nil)
                default:
                    EDLog("Export session completed with \(exportSession.status) status. Not handled.")
                    completion(nil)
                }
            }
        }
