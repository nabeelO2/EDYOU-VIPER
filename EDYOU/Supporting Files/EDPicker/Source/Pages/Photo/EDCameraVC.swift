...
internal final class EDCameraVC: UIViewController, UIGestureRecognizerDelegate, EDPermissionCheckable {
    var didCapturePhoto: ((UIImage) -> Void)?
    let v: EDCameraView!

    private let photoCapture = EDPhotoCaptureHelper()
    ...
    internal required init() {
        self.v = EDCameraView(overlayView: EDConfig.overlayView)
        super.init(nibName: nil, bundle: nil)

        title = EDConfig.wordings.cameraTitle
        navigationController?.navigationBar.setTitleFont(font: EDConfig.fonts.navigationBarTitleFont)
        
        EDDeviceOrientationHelper.shared.startDeviceOrientationNotifier { _ in }
    }
    ...
    deinit {
        EDDeviceOrientationHelper.shared.stopDeviceOrientationNotifier()
    }
    ...
    func cropImageToSquare(_ image: UIImage) -> UIImage {
        let orientation: UIDeviceOrientation = EDDeviceOrientationHelper.shared.currentDeviceOrientation
