...
internal class EDCameraView: UIView, UIGestureRecognizerDelegate {
    ...
    convenience init(overlayView: UIView? = nil) {
        ...
        // Style
        backgroundColor = EDConfig.colors.photoVideoScreenBackgroundColor
        previewViewContainer.backgroundColor = UIColor.ypLabel
        timeElapsedLabel.style { l in
            l.textColor = .white
            l.text = "00:00"
            l.isHidden = true
            l.font = EDConfig.fonts.cameraTimeElapsedFont
        }
        progressBar.style { p in
            p.trackTintColor = .clear
            p.tintColor = .ypSystemRed
        }
        flashButton.setImage(EDConfig.icons.flashOffIcon, for: .normal)
        flipButton.setImage(EDConfig.icons.loopIcon, for: .normal)
        shotButton.setImage(EDConfig.icons.capturePhotoImage, for: .normal)
    }
}
