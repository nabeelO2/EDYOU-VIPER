...
extension EDLibraryVC {
    var isLimitExceeded: Bool { return selectedItems.count >= EDConfig.library.maxNumberOfItems }
    ...
    @objc func handleLongPress(longPressGR: UILongPressGestureRecognizer) {
        if isMultipleSelectionEnabled || isProcessing || EDConfig.library.maxNumberOfItems <= 1 {
            return
        }
    ...
    func addToSelection(indexPath: IndexPath) {
        ...
        let newSelection = EDLibrarySelection(index: indexPath.row, assetIdentifier: asset.localIdentifier)
...
extension EDLibraryVC: UICollectionViewDataSource {
    public func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return mediaManager.fetchResult?.count ?? 0
    }
}

extension EDLibraryVC: UICollectionViewDelegate {
    ...
    public func collectionView(_ collectionView: UICollectionView,
        ...
        cell.multipleSelectionIndicator.selectionColor =
            EDConfig.colors.multipleItemsSelectedCircleColor ?? EDConfig.colors.tintColor
        ...
        cell.durationLabel.text = isVideo ? EDHelper.formattedStrigFrom(asset.duration) : ""
        ...
        if let index = selectedItems.firstIndex(where: { $0.assetIdentifier == asset.localIdentifier }) {
            ...
            if currentSelection.index < 0 {
                selectedItems[index] = EDLibrarySelection(index: indexPath.row,
...
extension EDLibraryVC: UICollectionViewDelegateFlowLayout {
    public func collectionView(_ collectionView: UICollectionView,
                               layout collectionViewLayout: UICollectionViewLayout,
                               sizeForItemAt indexPath: IndexPath) -> CGSize {
        let margins = EDConfig.library.spacingBetweenItems * CGFloat(EDConfig.library.numberOfItemsInRow - 1)
        let width = (collectionView.frame.width - margins) / CGFloat(EDConfig.library.numberOfItemsInRow)
    ...
    public func collectionView(_ collectionView: UICollectionView,
        ...
        return EDConfig.library.spacingBetweenItems
    }
