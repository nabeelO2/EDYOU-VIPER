...
internal final class EDLibraryVC: UIViewController, EDPermissionCheckable {
    internal weak var delegate: EDLibraryViewDelegate?
    internal var v = EDLibraryView(frame: .zero)
    internal var isProcessing = false // true if video or image is in processing state
    internal var selectedItems = [EDLibrarySelection]()
    ...
    required init() {
        ...
        title = EDConfig.wordings.libraryTitle
    }
    ...
    func initialize() {
        ...
        v.assetViewContainer.multipleSelectionButton.isHidden = !(EDConfig.library.maxNumberOfItems > 1)
        v.maxNumberWarningLabel.text = String(format: EDConfig.wordings.warningMaxItemsLimit,
											  EDConfig.library.maxNumberOfItems)
        
        if let preselectedItems = EDConfig.library.preselectedItems,
           !preselectedItems.isEmpty {
            selectedItems = preselectedItems.compactMap { item -> EDLibrarySelection? in
                ...
                return EDLibrarySelection(index: -1, assetIdentifier: asset.localIdentifier)
            }
        ...
        if EDConfig.library.defaultMultipleSelection || selectedItems.count > 1 {
            toggleMultipleSelection()
        }
    }

    func setAlbum(_ album: EDAlbum) {
        title = album.title
    ...
    public override func viewDidAppear(_ animated: Bool) {
        ...
        if !EDConfig.library.onlySquare && v.assetZoomableView.contentSize == CGSize(width: 0, height: 0) {
    ...
    func toggleMultipleSelection() {
        ...
        if EDConfig.library.minNumberOfItems > 1 && isMultipleSelectionEnabled {
            print("Selected minNumberOfItems greater than one :\(EDConfig.library.minNumberOfItems). Don't deselecting multiple selection.")
            return
        }
        ...
        if isMultipleSelectionEnabled {
            let needPreselectItemsAndNotSelectedAnyYet = selectedItems.isEmpty && EDConfig.library.preSelectItemOnMultipleSelection
            ...
            if needPreselectItemsAndNotSelectedAnyYet,
               ...
               let asset = mediaManager.getAsset(at: currentlySelectedIndex) {
                selectedItems = [
                    EDLibrarySelection(index: currentlySelectedIndex,
    ...
    func refreshMediaRequest() {
        ...
        let firstAsset = mediaManager.getAsset(at: 0) {
            ...
            if !isMultipleSelectionEnabled && EDConfig.library.preSelectItemOnMultipleSelection {
    ...
    func buildPHFetchOptions() -> PHFetchOptions {
        // Sorting condition
        if let userOpt = EDConfig.library.options {
            return userOpt
        }

        let options = PHFetchOptions()
        options.sortDescriptors = [NSSortDescriptor(key: "creationDate", ascending: false)]
        options.predicate = EDConfig.library.mediaType.predicate()
        return options
    }
    ...
    private func fitsVideoLengthLimits(asset: PHAsset) -> Bool {
        ...
        let tooLong = floor(asset.duration) > EDConfig.video.libraryTimeLimit
        let tooShort = floor(asset.duration) < EDConfig.video.minimumTimeLimit
        
        if tooLong || tooShort {
            DispatchQueue.main.async {
                let alert = tooLong ? EDAlert.videoTooLongAlert(self.view) : EDAlert.videoTooShortAlert(self.view)
    ...
    internal func fetchStoredCrop() -> EDLibrarySelection? {
    ...
    private func fetchVideoAndApplySettings(for asset: PHAsset,
        ...
        if EDConfig.video.automaticTrimToTrimmerMaxDuration {
            fetchVideoAndCropWithDuration(for: asset,
                                          withCropRect: resultCropRect,
                                          duration: EDConfig.video.trimmerMaxDuration,
                                          callback: callback)
        } else {
            delegate?.libraryViewDidTapNext()
    ...
    public func selectedMedia(photoCallback: @escaping (_ photo: EDMediaPhoto) -> Void,
                              videoCallback: @escaping (_ videoURL: EDMediaVideo) -> Void,
                              multipleItemsCallback: @escaping (_ items: [EDMediaItem]) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            if self.isMultipleSelectionEnabled && self.selectedItems.count > 1 {
                ...
                var resultMediaItems: [EDMediaItem] = []
                ...
                for asset in selectedAssets {
                    ...
                    case .image:
                        self.fetchImageAndCrop(for: asset.asset, withCropRect: asset.cropRect) { image, exifMeta in
                            let photo = EDMediaPhoto(image: image.resizedImageIfNeeded(),
													 exifMeta: exifMeta, asset: asset.asset)
                            resultMediaItems.append(EDMediaItem.photo(p: photo))
                            asyncGroup.leave()
                        }
                    ...
                    case .video:
                        self.fetchVideoAndApplySettings(for: asset.asset,
                            ...
                            if let videoURL = videoURL {
                                let videoItem = EDMediaVideo(thumbnail: thumbnailFromVideoPath(videoURL),
                                                             videoURL: videoURL, asset: asset.asset)
                                resultMediaItems.append(EDMediaItem.video(v: videoItem))
                            } else {
                                EDLog("Problems with fetching videoURL.")
                            }
                            asyncGroup.leave()
                        }
                    default:
                ...
                case .video:
                    self.fetchVideoAndApplySettings(for: asset, callback: { videoURL in
                        DispatchQueue.main.async {
                            if let videoURL = videoURL {
                                self.delegate?.libraryViewFinishedLoading()
                                let video = EDMediaVideo(thumbnail: thumbnailFromVideoPath(videoURL),
                ...
                @unknown default:
                    EDLog("unknown default reached. Check code.")
                }
                return
            }
        }
