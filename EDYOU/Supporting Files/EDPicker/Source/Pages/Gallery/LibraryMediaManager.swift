...
class LibraryMediaManager {
    ...
    func fetchVideoUrlAndCropWithDuration(for videoAsset: PHAsset,
        ...
        imageManager?.requestAVAsset(forVideo: videoAsset, options: videosOptions) { asset, _, _ in
            do {
                guard let asset = asset else { EDLog("Don't have the asset"); return }
                ...
                guard let videoTrack = asset.tracks(withMediaType: AVMediaType.video).first,
                    ...
                    EDLog("Problems with video track")
                    return
                        ...
                        DispatchQueue.main.async {
                            ...
                            case .completed:
                                if let url = session.outputURL {
                                    ...
                                    EDLog("Don't have URL.")
                                    callback(nil)
                                }
                            case .failed:
                                EDLog("Export of the video failed : \(String(describing: session.error))")
                                callback(nil)
                            default:
