...
class EDVideoCaptureHelper: NSObject {
    ...
    public func zoom(began: Bool, scale: CGFloat) {
        ...
        do {
            ...
            maxAvailableVideoZoomFactor = min(maxAvailableVideoZoomFactor, EDConfig.maxCameraZoomFactor)
    ...
    public func startRecording() {
        let outputURL = EDVideoProcessor.makeVideoPathURL(temporaryFolder: true, fileName: "recordedVideoRAW")
    ...
    private func setupCaptureSession() {
        ...
        let cameraPosition: AVCaptureDevice.Position = EDConfig.usesFrontCamera ? .front : .back
        ...
        if let videoInput = videoInput {
            ...
            if let sizeLimit = EDConfig.video.recordingSizeLimit {
                videoOutput.maxRecordedFileSize = sizeLimit
            }
            videoOutput.minFreeDiskSpaceLimit = EDConfig.video.minFreeDiskSpaceLimit
            if EDConfig.video.fileType == .mp4,
               EDConfig.video.recordingSizeLimit != nil {
    ...
    func tick() {
        ...
        var progress: Float
        if let recordingSizeLimit = EDConfig.video.recordingSizeLimit {
...
extension EDVideoCaptureHelper: AVCaptureFileOutputRecordingDelegate {
    ...
    public func fileOutput(_ captureOutput: AVCaptureFileOutput,
        ...
        if EDConfig.onlySquareImagesFromCamera {
            EDVideoProcessor.cropToSquare(filePath: outputFileURL) { [weak self] url in
