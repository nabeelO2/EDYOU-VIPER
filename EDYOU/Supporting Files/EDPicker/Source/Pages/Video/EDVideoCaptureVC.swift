...
internal class EDVideoCaptureVC: UIViewController, EDPermissionCheckable {
    var didCaptureVideo: ((URL) -> Void)?
    
    private let videoHelper = EDVideoCaptureHelper()
    private let v = EDCameraView(overlayView: nil)
    ...
    required init() {
        ...
        title = EDConfig.wordings.videoTitle
    ...
    func start() {
        self.videoHelper.start(previewView: v.previewViewContainer,
                               withVideoRecordingLimit: EDConfig.video.recordingTimeLimit) { [weak self] in
    ...
    private func setupButtons() {
        v.flashButton.setImage(EDConfig.icons.flashOffIcon, for: .normal)
        v.flipButton.setImage(EDConfig.icons.loopIcon, for: .normal)
        v.shotButton.setImage(EDConfig.icons.captureVideoImage, for: .normal)
    }
    ...
    func focusTapped(_ recognizer: UITapGestureRecognizer) {
        ...
        EDHelper.configureFocusView(v.focusView)
        v.addSubview(v.focusView)
        EDHelper.animateFocusView(v.focusView)
    }
    ...
    private func updateUIWith(state: ViewState) {
        func flashImage(for torchMode: FlashMode) -> UIImage {
            ...
            case .on: return EDConfig.icons.flashOnIcon
            case .off: return EDConfig.icons.flashOffIcon
            case .auto: return EDConfig.icons.flashAutoIcon
            }
        }
        ...
        v.shotButton.setImage(state.isRecording ? EDConfig.icons.captureVideoOnImage : EDConfig.icons.captureVideoImage,
                              for: .normal)
        v.flipButton.isEnabled = !state.isRecording
        v.progressBar.progress = state.progress
        v.timeElapsedLabel.text = EDHelper.formattedStrigFrom(state.timeElapsed)
    ...
    private func flashModeFrom(videoHelper: EDVideoCaptureHelper) -> FlashMode {
