...
public final class SelectBoxView: UIControl {
    ...
    private func drawTextLayer() {
        ...
        textLayer.frame = CGRect(
            x: (hx_width - textWidth) * 0.5,
            y: (hx_height - textHeight) * 0.5,
            width: textWidth,
            height: textHeight
        )
    ...
    public override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        if isUserInteractionEnabled && CGRect(x: -15, y: -15, width: hx_width + 30, height: hx_height + 30).contains(point) {
            return self
        }
