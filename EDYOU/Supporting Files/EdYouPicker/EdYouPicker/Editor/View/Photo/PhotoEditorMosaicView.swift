    ...
    @objc func panGesureRecognizerClick(panGR: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            if type == .mosaic {
                ...
                mosaicPoints.append(CGPoint(x: point.x / hx_width, y: point.y / hx_height))
            }
        case .changed:
            let point = panGR.location(in: self)
            ...
            if type == .mosaic {
                ...
                if let lastPoint = mosaicPoints.last,
                   (abs(newPoint.x - lastPoint.x * hx_width) <= 2 || abs(newPoint.y - lastPoint.y * hx_width) <= 2) {
                    return
                }
                var angle: CGFloat = 0
                if let startPoint = mosaicPoints.last {
                    angle = getAngleBetweenPoint(
                        startPoint: CGPoint(
                            x: startPoint.x * hx_width,
                            y: startPoint.y * hx_height
                        ),
                        endPoint: newPoint
                    )
                }
                ...
                if let colorLayer = createSmearLayer(at: newPoint, image: image, imageWidth: imageWidth, angle: angle) {
                    ...
                    mosaicPoints.append(CGPoint(x: newPoint.x / hx_width, y: newPoint.y / hx_height))
                    didChanged = true
