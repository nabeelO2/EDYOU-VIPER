...
class PhotoEditorView: UIScrollView {
    ...
    lazy var imageResizerView: EditorImageResizerView = {
        ...
        imageResizerView.imageView.drawView.lineColor = brush.colors[brush.defaultColorIndex].hx_Color
    ...
    var drawColorHex: String = "#ffffff" {
        didSet { imageResizerView.imageView.drawView.lineColor = drawColorHex.hx_Color }
    }
    ...
    func updateImageViewFrame() {
        ...
        let imageX: CGFloat = 0
        var imageY: CGFloat = 0
        if imageHeight < hx_height {
            imageY = (hx_height - imageHeight) * 0.5
            imageResizerView.setViewFrame(CGRect(x: 0, y: -imageY, width: hx_width, height: hx_height))
        }else {
...
//        var inputImage = imageResizerView.imageView.image
        let viewWidth = imageResizerView.imageView.hx_width
        let viewHeight = imageResizerView.imageView.hx_height
    ...
    func getVideoCropData() -> VideoEditorCropSizeData {
        let toRect = imageResizerView.getCroppingRect()
        let viewSize = imageResizerView.imageView.hx_size
        let cropRect = CGRect(
...
extension PhotoEditorView: UIScrollViewDelegate {
    ...
    func scrollViewDidZoom(_ scrollView: UIScrollView) {
        ...
        let offsetX = (scrollView.hx_width > scrollView.contentSize.width) ?
            (scrollView.hx_width - scrollView.contentSize.width) * 0.5 : 0
        let offsetY = (scrollView.hx_height > scrollView.contentSize.height) ?
            (scrollView.hx_height - scrollView.contentSize.height) * 0.5 : 0
