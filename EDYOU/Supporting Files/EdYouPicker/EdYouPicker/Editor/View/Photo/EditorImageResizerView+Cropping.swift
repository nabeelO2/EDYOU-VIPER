...
extension EditorImageResizerView {
                    ...
                    if !self.isOriginalRatio {
                        let offset = CGPoint(
                            x: -self.scrollView.contentInset.left +
                                (
                                self.imageView.hx_width * 0.5 - maskViewFrame.width * 0.5
                                ),
                            y: -self.scrollView.contentInset.top +
                                (
                                self.imageView.hx_height * 0.5 - maskViewFrame.height * 0.5
                                )
                        )
                ...
                if !isOriginalRatio {
                    let offset = CGPoint(
                        x: -scrollView.contentInset.left +
                            (
                                imageView.hx_width * 0.5 - maskViewFrame.width * 0.5
                            ),
                        y: -scrollView.contentInset.top + (
                            imageView.hx_height * 0.5 - maskViewFrame.height * 0.5
                        )
                    )
    ...
    func cancelCropTime(_ animated: Bool) {
        ...
        if hasCropping {
            ...
            let rectW = maxWidth
            let scale = maxWidth / controlView.hx_width
            let rectH = oldMaskRect.height * scale
            var rectY: CGFloat = 0
            if rectH < containerView.hx_height {
                rectY = (containerView.hx_height - rectH) * 0.5
            }
    ...
    func finishCropping(_ animated: Bool, completion: (() -> Void)?, updateCrop: Bool = true) {
        ...
        let fromSize = getExactnessSize(imageView.hx_size)
        let toSize = getExactnessSize(controlView.hx_size)
        ...
        let rectW = maxWidth
        let scale = maxWidth / controlView.hx_width
        let rectH = controlView.hx_height * scale
        var rectY: CGFloat = 0
        if rectH < containerView.hx_height {
            rectY = (containerView.hx_height - rectH) * 0.5
        }
    ...
    func canReset() -> Bool {
        ...
        if controlView.hx_size.equalTo(.zero) {
            // 裁剪框大小还未初始化时
            return false
        }
        if (isFixedRatio && cropConfig.aspectRatios.isEmpty && !cropConfig.resetToOriginal) || cropConfig.isRoundCrop {
    ...
    func rotate(_ isClock_wise: Bool = false) {
        ...
        if !controlView.fixedRatio {
            ...
            var maskHeight = controlView.hx_width * (maxWidth / controlView.hx_height)
