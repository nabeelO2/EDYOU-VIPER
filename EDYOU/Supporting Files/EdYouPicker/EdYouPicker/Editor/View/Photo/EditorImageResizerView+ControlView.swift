...
extension EditorImageResizerView: EditorImageResizerControlViewDelegate {
    ...
    func controlView(didChanged controlView: EditorImageResizerControlView, _ rect: CGRect) {
        ...
        case .up, .down:
            imageViewWidth = imageView.hx_width
            imageViewHeight = imageView.hx_height
        case .left, .right:
            imageViewWidth = imageView.hx_height
            imageViewHeight = imageView.hx_width
        }
    ...
    func adjustmentViews(_ animated: Bool, showMaskShadow: Bool = true) {
        ...
        /// 裁剪框的宽度
        var rectW = containerView.hx_width - contentInsets.left - contentInsets.right
        /// 裁剪框高度
        var rectH = rectW / controlView.hx_width * controlView.hx_height
        if rectH > maxHeight {
            /// 裁剪框超过最大高度就进行缩放
    ...
    func checkZoomOffset(
        ...
        switch getImageOrientation() {
        case .up:
            maxOffsetX = scrollView.contentSize.width - scrollView.hx_width + scrollCotentInset.left
            maxOffsetY = scrollView.contentSize.height - scrollView.hx_height + scrollCotentInset.bottom
        case .right:
            maxOffsetX = scrollView.contentSize.width - scrollView.hx_height + scrollCotentInset.right
            maxOffsetY = scrollView.contentSize.height - scrollView.hx_width + scrollCotentInset.bottom
        case .down:
            maxOffsetX = scrollView.contentSize.width - scrollView.hx_width + scrollCotentInset.left
            maxOffsetY = scrollView.contentSize.height - scrollView.hx_height + scrollCotentInset.bottom
        case .left:
            maxOffsetX = scrollView.contentSize.width - scrollView.hx_height + scrollCotentInset.right
            maxOffsetY = scrollView.contentSize.height - scrollView.hx_width + scrollCotentInset.top
        }
