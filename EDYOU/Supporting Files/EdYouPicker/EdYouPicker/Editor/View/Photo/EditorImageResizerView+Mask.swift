...
extension EditorImageResizerView {
    ...
    func getMaskViewFrame(_ isBase: Bool = false) -> CGRect {
        ...
        let maskWidth = isBase ? baseImageSize.width * zoomScale : imageView.hx_width * zoomScale
        let maskHeight = isBase ? baseImageSize.height * zoomScale : imageView.hx_height * zoomScale
        let maskX = (
            containerView.hx_width - contentInsets.left - contentInsets.right - maskWidth
        ) * 0.5 + contentInsets.left
        let maskY = (
            containerView.hx_height - contentInsets.top - contentInsets.bottom - maskHeight
