...
class PhotoEditorFilterView: UIView {
    ...
    override func layoutSubviews() {
        ...
        collectionView.frame = CGRect(x: 0, y: 0, width: hx_width, height: 120)
        ...
        editView.frame = .init(x: 0, y: 0, width: hx_width, height: 120)
        bottomView.frame = CGRect(x: 0, y: collectionView.frame.maxY, width: hx_width, height: 30)
        filterButton.frame = .init(x: 0, y: 0, width: hx_width * 0.5, height: 30)
        editButton.frame = .init(x: hx_width * 0.5, y: 0, width: hx_width * 0.5, height: 30)
        bottomLineView.hx_y = filterButton.hx_height
        setupBottomLineViewFrame()
    }
    
    func setupBottomLineViewFrame() {
        if !filterButton.isEnabled {
            bottomLineView.hx_width = (filterButton.titleLabel?.textWidth ?? 0) + 2
            bottomLineView.centerX = filterButton.centerX
            collectionView.isHidden = false
            editView.isHidden = true
        }else {
            bottomLineView.hx_width = (editButton.titleLabel?.textWidth ?? 0) + 2
...
extension PhotoEditorFilterView: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(
        ...
        if filter.isOriginal {
            let image = image == nil ? originalImage : image
...
extension PhotoEditorFilterView: PhotoEditorFilterViewCellDelegate {
    func filterViewCell(fetchFilter cell: PhotoEditorFilterViewCell) -> UIImage? {
        ...
        let imageOrg = image == nil ? originalImage : image
        if let handler = filterInfo.filterHandler, let image = imageOrg?.ci_Image {
            return handler(
                image,
...
class PhotoEditorFilterViewCell: UICollectionViewCell {
    ...
    func updateFilter() {
        ...
        if !filter.isOriginal {
             let image = delegate?.filterViewCell(fetchFilter: self)
    ...
    override func layoutSubviews() {
        super.layoutSubviews()
        imageView.frame = CGRect(x: 4, y: 4, width: hx_width - 8, height: hx_width - 8)
        editButton.frame = imageView.frame
        selectedView.frame = CGRect(x: 0, y: 0, width: hx_width, height: hx_width)
        let filterNameY = imageView.frame.maxY + 12
        var filterNameHeight = filterNameLb.text?.height(ofFont: filterNameLb.font, maxWidth: hx_width) ?? 15
        if filterNameHeight > hx_height - filterNameY {
            filterNameHeight = hx_height - filterNameY
        }
        filterNameLb.frame = CGRect(x: 0, y: filterNameY, width: hx_width, height: filterNameHeight)
        parameterLb.hx_y = filterNameLb.frame.maxY + 2
        parameterLb.hx_width = hx_width
        parameterLb.hx_height = 12
    }
...
class PhotoEditorFilterEditViewCell: UICollectionViewCell {
    ...
    override func layoutSubviews() {
        super.layoutSubviews()
        bgView.frame = .init(x: 4, y: 4, width: hx_width - 8, height: hx_width - 8)
        bgView.layer.cornerRadius = bgView.hx_width * 0.5
        imageView.hx_size = .init(width: 20, height: 20)
        imageView.centerX = bgView.hx_width * 0.5
        imageView.centerY = bgView.hx_height * 0.5
        let titleY = bgView.frame.maxY + 12
        var titleHeight = titleLb.textHeight
        if titleHeight > hx_height - titleY {
            titleHeight = hx_height - titleY
        }
        titleLb.frame = CGRect(x: 0, y: titleY, width: hx_width, height: titleHeight)
        
        parameterLb.hx_y = titleLb.frame.maxY + 2
        parameterLb.hx_width = hx_width
        parameterLb.hx_height = 12
    }
