...
extension EditorImageResizerView: UIScrollViewDelegate {
    ...
    func getScrollViewContentInset(_ rect: CGRect, _ isOld: Bool = false) -> UIEdgeInsets {
        ...
        case .up:
            ...
            let bottom: CGFloat = containerView.hx_height - rect.maxY
            var left: CGFloat
            var right: CGFloat
            if isOld ? oldMirrorType == .horizontal : mirrorType == .horizontal {
                left = containerView.hx_width - rect.maxX
                right = rect.minX
            }else {
                left = rect.minX
                right = containerView.hx_width - rect.maxX
            }
            return UIEdgeInsets(top: top, left: left, bottom: bottom, right: right)
        case .left:
            var top = rect.minX
            var bottom = containerView.hx_width - rect.maxX
            let left = containerView.hx_height - rect.maxY
            let right = rect.minY
            if isOld ? oldMirrorType == .horizontal : mirrorType == .horizontal {
                top = containerView.hx_width - rect.maxX
                bottom = rect.minX
            }else {
                top = rect.minX
                bottom = containerView.hx_width - rect.maxX
            }
            return UIEdgeInsets(top: top, left: left, bottom: bottom, right: right)
        case .down:
            let top = containerView.hx_height - rect.maxY
            let bottom = rect.minY
            var left = containerView.hx_width - rect.maxX
            var right = rect.minX
            if isOld ? oldMirrorType == .horizontal : mirrorType == .horizontal {
                left = rect.minX
                right = containerView.hx_width - rect.maxX
            ...
            return UIEdgeInsets(top: top, left: left, bottom: bottom, right: right)
        case .right:
            var top = containerView.hx_width - rect.maxX
            var bottom = rect.minX
            let left = rect.minY
            let right = containerView.hx_height - rect.maxY
            if isOld ? oldMirrorType == .horizontal : mirrorType == .horizontal {
                top = rect.minX
                bottom = containerView.hx_width - rect.maxX
            }else {
