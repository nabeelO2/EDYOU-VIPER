...
class EditorStickerItemView: UIView {
    ...
    init(item: EditorStickerItem, scale: CGFloat) {
        ...
        externalBorder.frame = CGRect(
            x: -margin * 0.5,
            y: -margin * 0.5,
            width: hx_width + margin,
            height: hx_height + margin
        )
            ...
            if let moveToCenter = delegate?.stickerItemView(self, moveToCenter: rect), !isDelete {
                ...
                if let view = keyWindow, moveToCenter,
                   let viewCenter = superview?.convert(
                    CGPoint(x: view.hx_width * 0.5, y: view.hx_height * 0.5),
                    from: view
                   ) {
...
//            layer.shadowOpacity = 0.3
    ...
    func update(pinchScale: CGFloat,
        ...
        var rect = frame
        rect.origin.x += (rect.width - contentView.hx_width) / 2
        rect.origin.y += (rect.height - contentView.hx_height) / 2
        rect.size.width = contentView.hx_width
        rect.size.height = contentView.hx_height
        frame = rect
        CATransaction.begin()
    ...
    func update(size: CGSize, isMirror: Bool = false) {
        ...
        contentView.hx_size = size
        contentView.center = CGPoint(x: hx_width * 0.5, y: hx_height * 0.5)
