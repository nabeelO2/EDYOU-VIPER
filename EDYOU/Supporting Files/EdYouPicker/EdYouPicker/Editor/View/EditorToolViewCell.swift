...
class EditorToolViewCell: UICollectionViewCell {
    ...
    lazy var btnBackgroundView: UIView = {
        let view = UIView()
    ...
    override func layoutSubviews() {
        ...
        btnBackgroundView.frame = bounds
       // button.frame = btnBackgroundView.frame
        if let image = button.image(for: .normal) {
            boxView.hx_x = hx_width * 0.5 + image.width * 0.5 - boxView.hx_width * 0.5
            boxView.hx_y = hx_height * 0.5 + image.height * 0.5 - boxView.hx_height + 3
        }
        btnBackgroundView.layer.borderWidth = 1
        btnBackgroundView.layer.borderColor = UIColor.lightGray.cgColor
        btnBackgroundView.layer.cornerRadius = btnBackgroundView.frame.width / 2
    }
}
