...
class EditorStickerView: UIView {
    ...
    lazy var trashView: EditorStickerTrashView = {
        ...
        view.hx_y = UIScreen.main.bounds.height
        view.alpha = 0
        return view
    }()
    ...
    func add(
        ...
        if item.text == nil && item.music == nil {
            ...
            var width = self.hx_width * self.scale
            var height = self.hx_height * self.scale
            ...
            pScale = min(ratio * width / itemView.hx_width, ratio * height / itemView.hx_height)
        }else if item.text != nil {
            pScale = min(
                min(
                    self.hx_width * self.scale - 40,
                    itemView.hx_width
                ) / itemView.hx_width,
                min(
                    self.hx_height * self.scale - 40,
                    itemView.hx_height
                ) / itemView.hx_height
            )
        }else {
    ...
    func showTrashView() {
        ...
        UIView.animate(withDuration: 0.25) {
            ...
            self.trashView.hx_y = UIScreen.main.bounds.height - UIDevice.bottomMargin - 20 - self.trashView.hx_height
            self.trashView.alpha = 1
        } completion: { _ in
            if !self.trashViewIsVisible {
                self.trashView.hx_y = UIScreen.main.bounds.height
    ...
    func stickerData() -> EditorStickerData? {
        ...
        for (index, subView) in subviews.enumerated() {
            if let itemView = subView as? EditorStickerItemView {
                ...
                let centerScale = CGPoint(x: itemView.centerX / hx_width, y: itemView.centerY / hx_height)
        ...
        for subView in subviews {
            if let itemView = subView as? EditorStickerItemView {
                ...
                let info = EditorStickerInfo(
                    ...
                    centerScale: CGPoint(x: itemView.centerX / hx_width, y: itemView.centerY / hx_height),
                    sizeScale: CGSize(
                        width: itemView.item.frame.width / hx_width,
                        height: itemView.item.frame.height / hx_height
                    ),
