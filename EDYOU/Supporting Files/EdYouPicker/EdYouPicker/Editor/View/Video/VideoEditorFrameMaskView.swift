...
class VideoEditorFrameMaskView: UIView {
    ...
    var validRect: CGRect = .zero {
        didSet {
            leftControl.frame = CGRect(x: validRect.minX - imageWidth * 0.5, y: 0, width: imageWidth, height: hx_height)
            rightControl.frame = CGRect(x: validRect.maxX - imageWidth * 0.5, y: 0, width: imageWidth, height: hx_height)
            drawMaskLayer()
        }
    }
    ...
    @objc func panGestureRecognizerAction(panGR: UIPanGestureRecognizer) {
        ...
        case 0:
            if point.x < minX {
                ...
                if rightControl.hx_x - point.x - imageWidth * 0.5 <= minWidth {
                    point.x = rightControl.hx_x - minWidth - imageWidth * 0.5
                }
            }
            point.y = 0
        ...
        case 1:
            let maxX = hx_width - imageWidth * 0.5 - validRectX
            if point.x > maxX {
                point.x = maxX
            }else {
