...
class VideoEditorSearchMusicView: UIView {
    ...
    lazy var searchView: SearchView = {
        ...
        searchIcon.hx_size = searchIcon.image?.size ?? .zero
    ...
    override func layoutSubviews() {
        ...
        bgView.frame = bounds
        topView.frame = CGRect(x: 0, y: 0, width: hx_width, height: 50)
        titleLb.hx_width = titleLb.text?.width(ofFont: titleLb.font, maxHeight: 50) ?? 0
        titleLb.hx_height = 50
        titleLb.centerX = hx_width * 0.5
        titleLb.hx_y = 0
        cancelButton.frame = CGRect(x: 12 + UIDevice.leftMargin, y: 0, width: 0, height: 50)
        cancelButton.hx_width = cancelButton.currentTitle?.width(ofFont: cancelButton.titleLabel!.font, maxHeight: 50) ?? 0
        finishButton.hx_width = (
            finishButton.currentTitle?.width(
                ofFont: finishButton.titleLabel!.font,
                maxHeight: 50
            ) ?? 0
        ) + 20
        if finishButton.hx_width < 55 {
            finishButton.hx_width = 55
        }
        finishButton.hx_height = 30
        finishButton.centerY = topView.hx_height * 0.5
        finishButton.hx_x = hx_width - UIDevice.rightMargin - 12 - finishButton.hx_width
        
        searchView.frame = CGRect(
            x: 12 + UIDevice.leftMargin,
            y: topView.frame.maxY + 12,
            width: hx_width - 24 - UIDevice.leftMargin - UIDevice.rightMargin,
            height: 35
        )
        ...
        collectionView.frame = CGRect(
            ...
            width: hx_width,
            height: hx_height - searchView.frame.maxY - 12
        )
    }
    ...
    class SearchView: UITextField {
        override func leftViewRect(forBounds bounds: CGRect) -> CGRect {
            ...
            return CGRect(
                x: (hx_height - rect.width) * 0.5,
                y: (hx_height - rect.height) * 0.5,
                width: rect.width,
                height: rect.height
            )
        }
...
extension VideoEditorSearchMusicView: UICollectionViewDataSource,
    ...
    func collectionView(
        ...
        CGSize(width: hx_width - collectionView.contentInset.left - collectionView.contentInset.right, height: 90)
    }
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        let offsetY = scrollView.contentOffset.y
        let contentHeight = scrollView.contentSize.height
        let maxOffsetY = contentHeight - scrollView.hx_height + scrollView.contentInset.bottom
    ...
    func setupCollectionInset() {
        ...
        if isLoading {
            if !isLoadMore {
                ...
                loadBgView.frame = CGRect(x: 0, y: -40, width: collectionView.hx_width - left - right, height: 40)
            }else {
                ...
                loadBgView.frame = CGRect(
                    ...
                    width: collectionView.hx_width - left - right,
                    height: 40
                )
            }
        }else {
...
extension VideoEditorSearchMusicView: UITextFieldDelegate {
    ...
    func startLoading(isMore: Bool) {
        ...
        loadingView.centerX = loadBgView.hx_width * 0.5
    ...
    func updateMoreLine() {
        let arcCenter = CGPoint(x: noMoreView.hx_width * 0.5, y: 10)
