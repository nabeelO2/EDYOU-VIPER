...
class VideoEditorCropView: UIView {
    ...
    func configData() {
        ...
        let maxWidth = hx_width - validRectX * 2 - imageWidth * 2
    ...
    override func layoutSubviews() {
        ...
        endTimeLb.frame = CGRect(x: hx_width - validRectX - 100, y: 0, width: 100, height: 20)
        collectionView.frame = CGRect(x: 0, y: 20, width: hx_width, height: itemHeight)
        frameMaskView.frame = collectionView.frame
        totalTimeLb.frame = CGRect(x: 0, y: collectionView.frame.maxY, width: 100, height: 20)
        totalTimeLb.centerX = hx_width * 0.5
...
//        let duration = getEndDuration() - CGFloat(time.seconds)
    ...
    func setLineAnimation(x: CGFloat, duration: TimeInterval) {
        ...
        progressLineView.frame = CGRect(x: x, y: collectionView.hx_y, width: 2, height: collectionView.hx_height)
        progressLineView.isHidden = false
        UIView.animate(withDuration: duration, delay: 0, options: [.curveLinear]) {
            self.progressLineView.hx_x = maxX
...
//            }
    ...
    func resetValidRect() {
        ...
        frameMaskView.validRect = CGRect(
            ...
            y: 0,
            width: hx_width - (validRectX + imgWidth) * 2,
            height: itemHeight
        )
    }
    func videoDuration(real: Bool = false) -> CGFloat {
        ...
        return CGFloat(Darwin.round(avAsset.duration.seconds))
    }
    ...
    func getRotateBeforeData(
        ...
        let validMaxWidth = hx_width - validInitialX * 2
    ...
    func updateTimeLabels() {
        ...
        var endDuration = Darwin.round(getEndDuration(real: true))
        var totalDuration = Darwin.round(endDuration - startDuration)
    ...
    func getStartDuration(real: Bool = false) -> CGFloat {
        ...
        let maxOfssetX = contentWidth - (collectionView.hx_width - collectionView.contentInset.left * 2.0)
    ...
    func stopScroll(_ offset: CGPoint?) {
        ...
        let maxOffsetX = contentWidth - (collectionView.hx_width - inset.left)
