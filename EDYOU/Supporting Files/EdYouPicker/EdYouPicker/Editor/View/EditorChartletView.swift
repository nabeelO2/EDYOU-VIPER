...
//        #endif
    ...
    override func layoutSubviews() {
        ...
        titleBgView.frame = CGRect(x: 0, y: 0, width: hx_width, height: 50)
        backButton.frame = CGRect(x: hx_width - 50 - UIDevice.rightMargin, y: 0, width: 50, height: 50)
        bgView.frame = CGRect(x: 0, y: titleBgView.frame.maxY, width: hx_width, height: hx_height - titleBgView.hx_height)
        titleView.frame = CGRect(x: 0, y: 0, width: hx_width, height: 50)
        titleView.contentInset = UIEdgeInsets(
            top: 5,
            left: 15 + UIDevice.leftMargin,
            bottom: 5,
            right: backButton.hx_width + UIDevice.rightMargin
        )
...
extension EditorChartletView: UICollectionViewDataSource,
    ...
    func currentIndex() -> Int {
        let offsetX = listView.contentOffset.x  + (listView.hx_width) * 0.5
        var currentIndex = Int(offsetX / listView.hx_width)
