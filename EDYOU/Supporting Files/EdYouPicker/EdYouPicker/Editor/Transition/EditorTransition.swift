    ...
    func pushTransition(
        ...
        if let editorVC = editorVC as? PhotoEditorViewController {
            if mode == .push {
                ...
                if editorVC.config[editorVC.currentPreviewIndex].fixedCropState {
...
#else
                if let image = editorVC.image, let imageView = editorVC.imageView() {
                    toRect = imageView.getTransitionImageViewFrame(
                        with: image.size,
                        viewSize: toVC.view.hx_size
                    )
                }else {
                    if let image = image, let imageView = editorVC.imageView() {
                        toRect = imageView.getTransitionImageViewFrame(
                            with: image.size,
                            viewSize: toVC.view.hx_size
                        )
                ...
                if let view = editorVC.imageView()?.imageResizerView{
                    ...
                    if editorVC.isFinishedBack {
                        ...
                        if view.layer.cornerRadius > 0 && view.layer.masksToBounds {
                            previewView.layer.cornerRadius = previewView.hx_width * 0.5
        ...
        }else if let editorVC = editorVC as? VideoEditorView {
            if mode == .push {
                editorVC.transitionCompletion = false
                ...
                if toRect.width < editorVC.hx_width {
                    toRect.origin.x = (editorVC.hx_width - toRect.width) * 0.5
                }
                if toRect.height < editorVC.hx_height {
                    toRect.origin.y = (editorVC.hx_height - toRect.height) * 0.5
                }
            }else {
            ...
            editorVC.backgroundColor = .clear
        }else {
        ...
        animate(
            ...
            if self.previewView.layer.cornerRadius > 0 {
                self.previewView.layer.cornerRadius = self.previewView.hx_width * 0.5
            }
            ...
            if self.mode == .push {
                ...
                if let editorVC = editorVC as? PhotoEditorViewController {
                    ...
                    editorVC.backgroundColor = .black
