...
extension VideoEditorView: EditorToolViewDelegate {
...
//        if let action = videoEditedResult{
//            action(nil)
//        }
        ...
        avAsset.loadValuesAsynchronously(
            ...
            DispatchQueue.global().async {
                if self.avAsset.statusOfValue(forKey: "tracks", error: nil) != .loaded {
                    self.showErrorHUD()
                    if let action = self.videoEditedResult{
                        action(nil)
                    }
                    return
                }
...
//                }
    ...
    func showErrorHUD() {
        ProgressHUD.hide(forView: self, animated: true)
        ProgressHUD.showWarning(addedTo: self, text: "Processing failed".localized, animated: true, delayHide: 1.5)
    }
...
extension VideoEditorView: EditorStickerTextViewControllerDelegate {
...
extension VideoEditorView: PhotoEditorCropToolViewDelegate {
