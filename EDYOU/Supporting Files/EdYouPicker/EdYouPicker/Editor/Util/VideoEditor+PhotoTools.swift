...
extension PhotoTools {
    ...
    /// Export edited video
    /// - Parameters:
    ///   - avAsset: The AVAsset data corresponding to the video
    ///   - outputURL: Specifies the address of the video export, when it is nil, it defaults to a temporary directory
    ///   - timeRang: The time area that needs to be cropped, without passing .zero
    ///   - cropSizeData: edit data
    ///   - audioURL: The audio address that needs to be added
    ///   - audioVolume: Audio volume to be added
    ///   - originalAudioVolume: Video Raw Audio Volume
    ///   - exportPreset: Export resolution
    ///   - videoQuality: Export quality
    ///   - completion: export complete
    @discardableResult
        ...
        do {
            ...
            if let exportSession = AVAssetExportSession(
                ...
                if supportedTypeArray.contains(AVFileType.mp4) {
                    ...
                    completion?(nil, PhotoError.error(type: .exportFailed, message: "Exporting this type of video is not supported"))
                    return nil
                }else {
                ...
                if videoQuality > 0 {
                    ...
                    if let urlAsset = avAsset as? AVURLAsset {
                        ...
                        maxSize = Int(Double(urlAsset.url.hx_fileSize) * scale)
                    }
...
//                    }
            ...
            completion?(nil, PhotoError.error(type: .exportFailed, message: "Export failed: " + error.localizedDescription))
        }
        return nil
    }
