...
class CameraResultViewController: UIViewController {
    ...
    var result: Any? //VideoEditResult, ImageEditResult
    
    init(result: Any, tintColor: UIColor) {
        self.result = result
    ...
    override func viewDidLayoutSubviews() {
        ...
        if UIDevice.isPad || !UIDevice.isPortrait {
            if UIDevice.isPad {
                ...
                let size = CGSize(width: view.hx_height * 16 / 9, height: view.hx_height)
                rect = CGRect(
                    x: (view.hx_width - size.width) * 0.5,
                    y: (view.hx_height - size.height) * 0.5,
                    width: size.width,
                    height: size.height
                )
            }
        }else {
            let size = CGSize(width: view.hx_width, height: view.hx_width / 9 * 16)
            rect = CGRect(
                x: (view.hx_width - size.width) * 0.5,
                y: (view.hx_height - size.height) * 0.5,
                width: size.width,
        ...
        doneButton.hx_width = doneWidth
        doneButton.hx_height = 33
        doneButton.hx_x = view.hx_width - doneWidth - 12 - UIDevice.rightMargin
        doneButton.centerY = view.hx_height - UIDevice.bottomMargin - 25
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        nav.navigationBar.setBackgroundImage(
            UIImage.image(
                for: .clear,
                havingSize: CGSize(width: view.hx_width, height: navHeight)
            ),
            for: .default
        )
