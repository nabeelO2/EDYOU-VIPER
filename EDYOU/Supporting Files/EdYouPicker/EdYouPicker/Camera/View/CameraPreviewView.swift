...
class CameraPreviewView: UIView {
    ...
    func showFilterName(_ filterName: String, _ isRight: Bool) {
        ...
        filterNameLb.centerX = !isRight ? hx_width * 0.5 + 50 :  hx_width * 0.5 - 50
        UIView.animate(withDuration: 0.25) {
            self.filterNameLb.alpha = 1
            self.filterNameLb.centerX = self.hx_width * 0.5
    ...
    func initialFocus() {
        let point = CGPoint(x: hx_width * 0.5, y: hx_height * 0.5)
        PhotoTools.focusAnimation(for: focusView, at: point)
        delegate?.previewView(self, tappedToFocusAt: captureDevicePoint(for: point))
    }
    ...
    override func layoutSubviews() {
        ...
        filterNameLb.frame = CGRect(x: 0, y: 0, width: hx_width, height: hx_height - 130)
    }
...
class CameraFocusView: UIView {
    ...
    lazy var lineLayer: CAShapeLayer = {
        ...
        path.move(to: CGPoint(x: hx_width * 0.5, y: 0))
        path.addLine(to: CGPoint(x: hx_width * 0.5, y: lineLength))
        
        path.move(to: CGPoint(x: hx_width * 0.5, y: hx_height - lineLength))
        path.addLine(to: CGPoint(x: hx_width * 0.5, y: hx_height))
        
        path.move(to: CGPoint(x: 0, y: hx_height * 0.5))
        path.addLine(to: CGPoint(x: lineLength, y: hx_height * 0.5))
        
        path.move(to: CGPoint(x: hx_width - lineLength, y: hx_height * 0.5))
        path.addLine(to: CGPoint(x: hx_width, y: hx_height * 0.5))
