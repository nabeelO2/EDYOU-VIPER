...
public class PhotoPickerViewController: BaseViewController {
    ...
    private lazy var emptyView: EmptyView = {
        let emptyView = EmptyView.init(frame: CGRect(x: 0, y: 0, width: view.hx_width, height: 0))
    ...
    public override func viewDidLayoutSubviews() {
        ...
        if let nav = navigationController {
            if nav.modalPresentationStyle == .fullScreen && UIDevice.isPortrait {
                if UIApplication.shared.isStatusBarHidden {
                    collectionTop = nav.navigationBar.hx_height + UIDevice.generalStatusBarHeight
                }else {
                    collectionTop = UIDevice.navigationBarHeight
                }
            }else {
                collectionTop = nav.navigationBar.hx_height
            }
        }
        ...
        if isMultipleSelect {
            ...
            collectionView.contentInset = UIEdgeInsets(
                top: collectionTop,
                left: 0,
                bottom: bottomView.hx_height + 0.5,
                right: 0
            )
        ...
        emptyView.center = CGPoint(
            x: collectionView.hx_width * 0.5,
            y: (collectionView.hx_height - collectionView.contentInset.top - collectionView.contentInset.bottom) * 0.5
        )
    }
