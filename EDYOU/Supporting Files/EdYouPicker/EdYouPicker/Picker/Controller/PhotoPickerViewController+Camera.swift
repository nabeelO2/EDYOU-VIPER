...
//        }
        ...
        imagePickerController.videoMaximumDuration = 60//camerConfig.videoMaximumDuration
        imagePickerController.videoQuality = .typeHigh//camerConfig.videoQuality
        imagePickerController.allowsEditing = true//camerConfig.allowsEditing
        imagePickerController.cameraDevice = .rear//camerConfig.cameraDevice
...
//            self.bottomView.didFinishButtonClick(button: self.bottomView.finishBtn)
            picker.pickerViewController?.openEditor(photoAsset, nil, [photoAsset])
...
extension PhotoPickerViewController: CameraControllerDelegate {
    public func cameraController(_ cameraController: CameraController, didFinishWithSelectedView result: [UIView]) {
...
//                mediaType = .video
//                let videoDuration = PhotoTools.getVideoDuration(videoURL: videoURL)
                if let edit = result as? PhotoEditResult{
                    mediaType = .image
                    photoAsset = .init(localImageAsset: .init(image:  edit.editedImage))
                }
                else  {
                    let edit = result as! VideoEditResult
                    mediaType = .video
                    photoAsset = .init(localVideoAsset: .init(videoURL: edit.editedURL, duration: edit.videoDuration))
                }
            ...
            if let asset = photoAsset{
                ...
                self.addedCameraPhotoAsset(
                    ...
                    self?.cameraControllerDismiss(canSelect)
                    picker.finishHandler?([asset], picker)
                }
            }
    ...
    func cameraControllerDismiss(_ canSelect: Bool) {
        if let picker = pickerController{
            guard  picker.config.selectMode == .single,
               config.finishSelectionAfterTakingPhoto,
               canSelect else {
                return
            }
