...
extension PhotoPickerViewController: UICollectionViewDelegate {
    ...
    func didSelectItem(
        ...
        if cell is PickerAddTextCell{
            //present picker cell
            let vc = AddTextController(nibName: "AddTextController", bundle: nil)
            vc.textHandler = { param in
                if let picker = self.pickerController{
                    picker.finishHandler?([param], picker)
...
//            videoEditorVC.backgroundColor = .cyan
            ...
            videoEditorVC.frame = self.view.bounds
            self.view.addSubview(videoEditorVC)
...
extension PhotoPickerViewController: UICollectionViewDelegateFlowLayout {
    public func collectionView(
        ...
        if config.showAssetNumber && (photoCount > 0 || videoCount > 0) {
            return CGSize(width: view.hx_width, height: 50)
        }
        return .zero
    }
}
