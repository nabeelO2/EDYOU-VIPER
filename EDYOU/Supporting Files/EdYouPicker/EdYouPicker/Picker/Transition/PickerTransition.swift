...
//                #endif
        ...
        if type == .push {
            if UIDevice.isPad && photoAsset?.mediaType == .video {
                rect = PhotoTools.transformImageSize(
                    ...
                    toViewSize: toVC.view.hx_size,
                    directions: [.horizontal]
                )
                if rect.width < toVC.view.hx_width {
                    rect.origin.x = (toVC.view.hx_width - rect.width) * 0.5
                }
                if rect.height < toVC.view.hx_height {
                    rect.origin.y = (toVC.view.hx_height - rect.height) * 0.5
                }
            }else {
                rect = PhotoTools.transformImageSize(
                    photoAsset?.imageSize ?? .zero,
            ...
            if pickerController.selectedAssetArray.isEmpty == false {
                ...
                if !pickerShowParompt {
                    ...
                    let maskView = UIView(
                        frame: CGRect(
                            x: 0, y: maskY,
                            width: contentView.hx_width, height: maskHeight
                        )
                    )
        ...
        if pickerShowParompt && !previewShowSelectedView {
            ...
            let maskView = UIView.init(frame: CGRect(x: 0, y: maskY, width: contentView.hx_width, height: maskHeight))
            maskView.backgroundColor = .white
            ...
            UIView.animate(withDuration: duration - 0.2, delay: 0, options: [.curveEaseIn]) {
                if previewVC.bottomView.mask != nil {
                    previewVC.bottomView.mask?.frame = CGRect(
                        x: 0, y: 0,
                        width: contentView.hx_width, height: maskHeight + 70
                    )
                }
                if pickerVC?.bottomView.mask != nil {
                    pickerVC?.bottomView.mask?.frame = CGRect(x: 0, y: 70, width: contentView.hx_width, height: maskHeight)
                }
            }
