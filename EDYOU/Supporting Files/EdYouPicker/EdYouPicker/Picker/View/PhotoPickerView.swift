...
open class PhotoPickerView: UIView {
    ...
    func setupOther() {
        ...
        emptyView.centerY = collectionView.hx_height * 0.5
        if scrollDirection == .horizontal && config.allowAddCamera {
            let cameraWidth = (hx_height - contentInset.top - contentInset.bottom) / 16 * 9
            let contentWidth = hx_width - contentInset.left - contentInset.right
            emptyView.centerX = cameraWidth + (contentWidth - cameraWidth) * 0.5
        }else {
            emptyView.centerX = collectionView.hx_width * 0.5
        }
