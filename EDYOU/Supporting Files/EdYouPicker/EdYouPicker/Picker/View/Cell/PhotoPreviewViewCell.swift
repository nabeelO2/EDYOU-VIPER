...
open class PhotoPreviewViewCell: UICollectionViewCell, UIScrollViewDelegate {
    ...
    func setupLandscapeContentSize() {
        ...
        var contentHeight = hx_height
        if contentWidth > hx_width {
            contentHeight = hx_width / contentWidth * contentHeight
            contentWidth = hx_width
            scrollView.maximumZoomScale = hx_height / contentHeight + 0.5
            scrollContentView.frame = CGRect(x: 0, y: 0, width: contentWidth, height: contentHeight)
            scrollView.contentSize = scrollContentView.hx_size
        }else {
    ...
    @objc func doubleTap(tap: UITapGestureRecognizer) {
        if scrollView.zoomScale > 1 {
            ...
            let zoomWidth = hx_width / maximumZoomScale
            let zoomHeight = hx_height / maximumZoomScale
            scrollView.zoom(
                to: CGRect(
    ...
    public func scrollViewDidZoom(_ scrollView: UIScrollView) {
        let offsetX = (scrollView.hx_width > scrollView.contentSize.width) ?
            (scrollView.hx_width - scrollView.contentSize.width) * 0.5 : 0.0
        let offsetY = (scrollView.hx_height > scrollView.contentSize.height) ?
            (scrollView.hx_height - scrollView.contentSize.height) * 0.5 : 0.0
