...
public class PreviewVideoControlViewCell: PreviewVideoViewCell, PreviewVideoSliderViewDelegate {
    ...
    public override func layoutSubviews() {
        ...
        sliderView.frame = CGRect(
            x: 0,
            y: hx_height - 50 - UIDevice.bottomMargin,
            width: hx_width,
            height: 50 + UIDevice.bottomMargin
        )
        
        maskLayer.frame = CGRect(x: 0, y: sliderView.hx_y - 20, width: hx_width, height: sliderView.hx_height + 20)
    }
...
class PreviewVideoSliderView: UIView, SliderViewDelegate {
    ...
    lazy var playButton: PlayButton = {
        ...
        button.hx_size = CGSize(width: 30, height: 40)
    ...
    override func layoutSubviews() {
        super.layoutSubviews()
        playButton.hx_x = 15 + UIDevice.leftMargin
        playButton.centerY = 25
        
        currentTimeLb.hx_x = playButton.frame.maxX + 5
        currentTimeLb.centerY = playButton.centerY
        
        totalTimeLb.hx_x = hx_width - totalTimeLb.hx_width - 15 - UIDevice.rightMargin
        totalTimeLb.centerY = currentTimeLb.centerY
        
        sliderView.hx_x = currentTimeLb.frame.maxX + 5
        sliderView.hx_width = totalTimeLb.hx_x - 5 - sliderView.hx_x
        sliderView.hx_height = hx_height
...
class PlayButton: UIControl {
    ...
    func updatePlay() {
        ...
        if isSelected {
            let leftLineStartPoint = CGPoint(x: hx_width * 0.5 - widthMargin * 0.5, y: hx_height * 0.5 - heightMargin * 0.5)
            ...
            let rightLineStartPoint = CGPoint(x: hx_width * 0.5 + widthMargin * 0.5, y: hx_height * 0.5 - heightMargin * 0.5)
            ...
            let startPoint = CGPoint(x: hx_width * 0.5 + widthMargin * 0.5, y: hx_height * 0.5)
...
class SliderView: UIView {
    ...
    @objc func panGestureRecognizerClick(pan: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            if rect.midX < 5 {
                rect.origin.x = -thumbView.hx_width * 0.5 + 5
            }
            if rect.midX > hx_width - 5 {
                rect.origin.x = hx_width - 5 - thumbView.hx_width * 0.5
            }
            value = (rect.midX - 5) / (hx_width - 10)
    ...
    var bufferValue: CGFloat = 0 {
        didSet {
            bufferView.hx_width = hx_width * bufferValue
        }
    }
    ...
    func setValue(
        ...
        if isAnimation {
            UIView.animate(
                ...
                self.thumbView.centerX = 5 + (self.hx_width - 10) * self.value
                self.trackView.hx_width = currentWidth
            }
        }else {
            thumbView.centerX = 5 + (hx_width - 10) * value
    ...
    override func layoutSubviews() {
        ...
        bufferView.frame = CGRect(x: 0, y: (hx_height - 3) * 0.5, width: hx_width * bufferValue, height: 3)
