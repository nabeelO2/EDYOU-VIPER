...
extension PhotoPickerView: UICollectionViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let offsetX = scrollView.contentOffset.x + collectionView.hx_width
        let point = CGPoint(x: offsetX, y: collectionView.hx_height * 0.5)
        ...
        var m = offsetX - cell.hx_x - cell.selectControl.hx_width - rightMargin
        if m < 0 {
            m = 0
        }
        if m > cell.hx_width - rightMargin - cell.selectControl.hx_width {
            m = cell.hx_width - rightMargin - cell.selectControl.hx_width
        }
        cell.selectControl.hx_x = m
    }
...
extension PhotoPickerView: UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(
        ...
        if scrollDirection == .vertical {
            ...
            let collectionWidth = collectionView.hx_width - contentInset.left - contentInset.right
            let itemWidth = (collectionWidth - space * (count - CGFloat(1))) / count
            return CGSize(width: itemWidth, height: itemWidth * 1.3)
        }
        let maxHeight = hx_height - contentInset.top - contentInset.bottom
        ...
        let maxWidth = min(hx_width - 60, maxHeight / 9 * 14)
        let photoAsset = getPhotoAsset(for: indexPath.item)
